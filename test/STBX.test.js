...
contract("STBXToken", (accounts) => {
    ...
    it("should be not whitelisted address", async () => {
        assert.equal(false, await contractInstance.isWhitelistedAddress(bob, { from: superadmin }));
    });

    it("owner should be whitelisted address", async () => {
        assert.equal(true, await contractInstance.isWhitelistedAddress(superadmin, { from: superadmin }));
    });

    it("add and remove from whitelist", async () => {
        await utils.shouldThrow(contractInstance.addAddressToWhitelist(alice, { from: bob }));
        await contractInstance.addWhitelister(bob, { from: superadmin });
        assert.equal(true, await contractInstance.isWhitelister(bob, { from: bob }));
        assert.equal(false, await contractInstance.isWhitelister(alice, { from: alice }));
        await contractInstance.addAddressToWhitelist(alice, { from: bob });
        assert.equal(true, await contractInstance.isWhitelistedAddress(alice, { from: superadmin }));
        await contractInstance.removeAddressFromWhitelist(alice, { from: bob });
        assert.equal(false, await contractInstance.isWhitelistedAddress(alice, { from: superadmin }));
    });
    ...
    it("freezing of funds", async () => {
        ...
        await utils.shouldThrow(contractInstance.transfer(alice, 900, { from: bob }));
    ...
    it("transfering of funds", async () => {
        ...
        await contractInstance.setTransferLimit(superadmin, 100000, { from: superadmin });
        ...
        await contractInstance.transferFunds(bob, superadmin, 10000, { from: superadmin });

        balance = await contractInstance.balanceOf(superadmin, { from: superadmin });
        assert.equal(10000000, balance);

        await contractInstance.transferFunds(superadmin, bob, 10000000, { from: alice });

        balance = await contractInstance.balanceOf(bob, { from: bob });
        assert.equal(10000000, balance);

        await contractInstance.splitOrMerge(1 * (10 ** 5), 2 * (10 ** 5), { from: superadmin });
        ...
        await contractInstance.splitOrMerge(2 * (10 ** 5), 3 * (10 ** 5), { from: superadmin });
    ...
    it("minting and burning of funds", async () => {
        ...
        await utils.shouldThrow(contractInstance.burn(bob, 27600, { from: superadmin }));
    });
    ...
    it("stock splits and reverse splits", async () => {
        ...
        await contractInstance.splitOrMerge(1 * (10 ** 5), 4 * (10 ** 5), { from: superadmin });
        ...
        await contractInstance.splitOrMerge(3 * (10 ** 5), 7 * (10 ** 5), { from: superadmin });

        balance = await contractInstance.balanceOf(bob, { from: bob });
        assert.equal(279999, balance);

        await contractInstance.splitOrMerge(4 * (10 ** 5), 1 * (10 ** 5), { from: superadmin });
        ...
        await contractInstance.splitOrMerge(7 * (10 ** 5), 4 * (10 ** 5), { from: superadmin });
        ...
        await contractInstance.splitOrMerge(8 * (10 ** 5), 1 * (10 ** 5), { from: superadmin });

        balance = await contractInstance.balanceOf(bob, { from: bob });
        assert.equal(312, balance);

        await contractInstance.splitOrMerge(5 * (10 ** 5), 3 * (10 ** 5), { from: superadmin });
    ...
    it("deciaml ratio splits and reverse splits", async () => {
        ...
        await contractInstance.splitOrMerge(1 * (10 ** 5), 133300, { from: superadmin });
    ...
    it("voter role", async () => {
        assert.equal(false, await contractInstance.isVoter(superadmin, { from: superadmin }));
    ...
    it("limits", async () => {
        ...
        await contractInstance.addLimiter(superadmin, { from: superadmin });
        await contractInstance.addAddressToWhitelist(bob, { from: superadmin });
        await utils.shouldThrow(contractInstance.transfer(bob, 100001, { from: superadmin }));

        limit = await contractInstance.getTransferLimit(superadmin, { from: superadmin });
        assert.equal(limit, 1000);

        allowedToTransfer = await contractInstance.getAllowedToTransfer(superadmin, { from: superadmin });
        ...
        await utils.shouldThrow(contractInstance.setTransferLimit(bob, 500, { from: bob }));
        ...
        allowedToTransfer = await contractInstance.getAllowedToTransfer(bob, { from: bob });
        assert.equal(allowedToTransfer, 1);

        await utils.shouldThrow(contractInstance.transfer(superadmin, 10, { from: bob }));
    });
});
